...
export class AddEditProductsPage implements OnInit {
  ...
  constructor(
    private _fb: FormBuilder, private _brands: BrandsService, private _size: SizesService,
    private _providers: ProvidersService, private _products: ProductsService,
    private _categories: CategoriesService, private _router: Router,
    private _toast: ToastService, private _tabs: TabsServices,
    private _dataSource: DataSourceService, private _api: ApiService) {
  ...
  ngOnInit() {
    ...
    if (this._dataSource.simpleObject?.id) {
      ...
      this.productForm.setControl('talles', this._fb.array(this._dataSource.simpleObject.talles));
  ...
  checkIfHasSpecifications() {
    if (this._dataSource.simpleObject.alto !== null ||
  ...
  post() {
    this._toast.sweetConfirm('Guardar', '¿Desea guardar el producto?')
      .then(res => {
        if (res)
  ...
  createForm() {
    this.productForm = this._fb.group({
      ...
      unidad: [1],
      proveedor: this._fb.group({ id: ['', [Validators.required]], proveedor: [''] }),
      marca: this._fb.group({ id: ['', [Validators.required]], marca: [''] }),
      categoria_uno: this._fb.group({ id: ['', [Validators.required]], categoria: [''] }),
      categoria_dos: this._fb.group({ id: ['', [Validators.required]], categoria: [''] }),
  ...
  redirect() {
    this._router.navigate(["/productos/tab-productos"])
      .then(() => this._tabs.setShowTable(true))
  }

  /** Obtención de datos  */
  getProviders() {
    this._providers.getAll().subscribe((res: Providers[]) => this.providers = res['data']);
  }

  getBrands() {
    this._brands.getAll().subscribe((res: Brands[]) => this.brands = res['data']);
  }

  getCategories() {
    this._categories.getAll().subscribe((res: Categories[]) => this.categories = res['data']);
  }

  getSizes() {
    this._size.getAll().subscribe((res: Sizes[]) => { this.sizesArray = res['data'] });
  }
  ...
  addSizeToTallesFormArray(size) {
    if (this.talles.value.some(el => el.id === size.id))
      ...
      this.talles.push(this._fb.control(size))
  ...
  get descriptionInvalid() {
    return this.productForm.get('descripcion').invalid && this.productForm.get('descripcion').touched;
  }

  get stockInvalid() {
    return this.productForm.get('stock_actual').invalid && this.productForm.get('stock_actual').touched;
  }
