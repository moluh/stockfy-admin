...
export class ExpensesComponent implements OnInit {
  ...
  delete(exp) {
    this._toast.sweetConfirm('¿Eliminar gasto?', 'Una vez eliminado no se podrá recuperar.')
      .then((res) => {
        if (res)
          this._expenses.delete(exp.id).subscribe(
            (res: any) => {
              if(res.ok) {
                ...
                this._api.handleError(res, 'Ocurrió un error', res.userMessage)
              }
  ...
  goTo(expense?) {
    if (expense)
      this._dataSource.simpleObject = expense;
    this._router.navigate([`add-edit-expenses`], { relativeTo: this._route, skipLocationChange: false })
