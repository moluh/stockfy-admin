...
export class TabProductsComponent implements OnInit {
  ...
  getPaginated() {
    this._products.getPaginated(this.pag.pageNro, this.pag.pageSize).subscribe(
  ...
  getPaginatedByState() {
    this._products
      .getPaginatedByState(this.pag.pageNro, this.pag.pageSize, this.isArchive)
      .subscribe(
        (res: Products[] | any) => this.setData(res),
        (err) => console.log(err)
      );
  ...
  getPaginatedAndFilter() {
    if (this.pag.text === "")
      return this._toast.toastAlert('Ingrese una palabra para filtrar', '');
      ...
      (event) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          if (event.status == 200) {
            ...
            this._toast.toastSuccess('¡Archivo agregado con éxito!', '');
            this.getPaginated();
          }
        }
      },
