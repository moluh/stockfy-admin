...
export class UsersService {
    ...
    public hasSuperAdminRole(roles: Role[]) {
        if (roles) return roles.some((r) => r.role === 'SUPERADMIN')
        return false
    }

    public hasAdminRole(roles: Role[]) {
        if (roles) return roles.some((r) => r.role === 'ADMIN')
        return false
    }

    public hasUserRole(roles: Role[]) {
        if (roles) return roles.some((r) => r.role === 'USUARIO')
        return false
    }
    ...
    public getPaginated(
        ...
        return this.http.get<Users[]>(`${this.url}s/paginado`, { params }).pipe(
            map((res) => {
                return <Users[]>res
            }),
    ...
    public getPaginatedByTxtAndFilter(
        ...
        return this.http
            .get<Users[]>(`${this.url}s/paginado/filter`, { params })
            .pipe(
                map((res) => {
                    return <Users[]>res
                }),
    ...
    //  ====================================================================================
    public getAll() {
        return this.http.get<Users[]>(`${this.url}s`, this.httpOptions).pipe(
            map((res) => {
                return <Users[]>res
            }),
            catchError(this.he.handleError)
    ...
    public update(user: Users): Observable<Users> {
        return this.http
            .put<Users>(`${this.url}/${user.id}`, user, this.httpOptions)
            .pipe(
                map((res) => {
                    return <Users>res
                }),
    ...
    public post(user: Users): Observable<Users> {
        ...
        user.activo = true
        return this.http
            .post<Users>(`${this.url}s`, user, this.httpOptions)
            .pipe(
                map((res) => {
                    return <Users>res
                })
    ...
    public delete(id: number): Observable<Users> {
        return this.http
            .delete<Users>(`${this.url}/${id}`, this.httpOptions)
            .pipe(
                map((res) => {
                    return <Users>res
                }),
